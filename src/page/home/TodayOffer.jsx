...
import { createTodayOffer, deleteTodayOffer, getTodayOffer } from "../../function/todayOffer";
    ...
    <div className="container">
                ...
                <div className="col-2">
                  <div className="pro">
                    ...
                    <label htmlFor={data._id}>
                      <img
                        style={{ width: "100%" }}
                        src={`http://172.232.78.160:8000/${data.images[0]}`}
                      />
                      <span>{data.name}</span>
                    </label>
                  </div>
              ...
              <div className="row">
                {preview.map((product) => (
                  <div className="col-3 card mx-1 p-2 mt-2" key={product._id}>
                    <img
                      style={{ width: "100%" }}
                      src={`http://172.232.78.160:8000/${product.images[0]}`}
                      alt={product.name}
                    />
                    <p>{product.name}</p>
                  </div>
      ...
      <div className="container today-offer p-4 mt-4">
        ...
        {offer?.products?.map((product) => (
              ...
              <button className="btn btn-danger" onClick={() => deleteOffer(offer._id)}>Delete</button>
      </div>
    </div>
  );
};
